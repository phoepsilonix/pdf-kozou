...
import {
  renderPage, getPdfInfo,
  getPageText, searchPage, getPageLinks,
  type PdfInfo, type PdfMetadata,
  type STextBlock, type PageLink,
} from "../lib/tauri";
...
function ptToMm(pt: number) { return (pt * 25.4 / 72).toFixed(1); }
  ...
  return (
    <div style={{
      ...
      {searchHits.map((hit, i) => {
        ...
        const isCurrent = currentHit && hit.page === currentHit.page &&
          hit.quad[0] === currentHit.quad[0] && hit.quad[1] === currentHit.quad[1];
        return (
          <div key={i} style={{
            position: "absolute",
            left: ulx, top: uly,
            width: urx - ulx, height: lly - uly,
    ...
    {links.map((link, i) => {
      ...
      return (
        <a key={i}
          ...
          onClick={link.dest_page != null ? e => { e.preventDefault(); onNavigate(link.dest_page!); } : undefined}
          style={{ position: "absolute", left: x0, top: y0, width: x1-x0, height: y1-y0, cursor: "pointer", display: "block" }}
          title={link.uri}
        />
      );
    })}
...
function MetaRow({ label, value }: { label: string; value?: string }) {
  ...
  return (
    <div style={ds.row}>
      ...
      <button style={{ ...ds.copyBtn, ...(copied ? ds.copyBtnDone : {}) }}
...
function InfoDrawer({ open, onClose, info, filePath, fileName }:
  { open: boolean; onClose: () => void; info: PdfInfo | null; filePath: string; fileName: string }) {
  ...
  const hasAnyMeta = !!(meta.title||meta.author||meta.subject||meta.keywords||meta.creator||meta.producer||meta.creation_date||meta.mod_date);
  ...
  const handleCopyAll = () => {
    ...
    navigator.clipboard.writeText(lines).then(() => { setAllCopied(true); setTimeout(() => setAllCopied(false), 1800); });
  };

  return (
    <div style={{ ...ds.drawer, transform: open ? "translateX(0)" : "translateX(100%)", pointerEvents: open ? "auto" : "none" }}>
      <div style={ds.drawerHead}>
        <span style={ds.drawerTitle}>ℹ ファイル情報</span>
        <div style={{ flex: 1 }} />
        <button style={{ ...ds.copyBtn, padding: "3px 10px", fontSize: 11 }} onClick={handleCopyAll}>
          {allCopied ? "✓ コピー済" : "全コピー"}
        </button>
      ...
      <div style={ds.drawerBody}>
        ...
        <div style={ds.row}>
          ...
          <button style={ds.copyBtn} onClick={() => navigator.clipboard.writeText(fileName)}>⎘</button>
        </div>
        <div style={ds.row}>
          <span style={ds.label}>パス</span>
          <span style={{ ...ds.value, wordBreak: "break-all", fontSize: 10 }}>{filePath}</span>
          <button style={ds.copyBtn} onClick={() => navigator.clipboard.writeText(filePath)}>⎘</button>
        </div>
        {info?.file_size != null && <div style={ds.row}><span style={ds.label}>サイズ</span><span style={ds.value}>{formatBytes(info.file_size)}</span></div>}
        {info?.page_count != null && <div style={ds.row}><span style={ds.label}>ページ数</span><span style={ds.value}>{info.page_count} ページ</span></div>}
        {pageSize && <div style={ds.row}><span style={ds.label}>ページサイズ</span><span style={ds.value}>{pageSize}</span></div>}
        ...
        {hasAnyMeta ? (<>
          ...
          <div style={{ color: "var(--c-textDim)", fontSize: 11, padding: "10px 0", textAlign: "center" }}>メタデータなし</div>
        )}
      </div>
    </div>
  );
...
function SearchBar({ path, totalPages, currentPage, onNavigate, onAllHits, onClose }: SearchBarProps) {
  ...
  const doSearch = useCallback(async (needle: string) => {
    // 前の検索をキャンセル
    searchRef.current?.abort();
    if (!needle.trim()) {
      setAllHits([]); setCurrent(0);
      onAllHits([]);
      return;
    }
  ...
  return (
    <div style={ss.bar}>
      <input style={ss.input} value={q}
        onChange={e => { setQ(e.target.value); doSearch(e.target.value); }}
        onKeyDown={e => { if (e.key === "Enter") go(e.shiftKey ? -1 : 1); if (e.key === "Escape") handleClose(); }}
      ...
      {allHits.length > 0 && <>
        <button style={ss.navBtn} onClick={() => go(-1)} title="前のヒット (Shift+Enter)">◀</button>
        <button style={ss.navBtn} onClick={() => go(1)} title="次のヒット (Enter)">▶</button>
      </>}
      <button style={ss.close} onClick={handleClose}>✕</button>
    </div>
  );
}

// ── メインコンポーネント ──────────────────────────────────────────────────────
interface Props { filePath?: string; pdfInfo?: PdfInfo; fileList?: FileEntry[]; }
...
export function ViewerPage({ filePath, pdfInfo, fileList = [] }: Props) {
  ...
  // ヒット位置へスクロールする共通処理
  const scrollToHit = useCallback((hit: GlobalHit) => {
  ...
  useEffect(() => {
    if (!isMulti) {
      ...
      else if (activePath) getPdfInfo(activePath).then(i => { setActiveInfo(i); setTotal(i.page_count); });
      return;
    }
    const p = fileList[activeIdx]?.path;
    if (p) getPdfInfo(p).then(i => { setActiveInfo(i); setTotal(i.page_count); });
  ...
  const getOrRender = useCallback(async (path: string, page: number): Promise<string> => {
    ...
    const b64 = await renderPage(path, page, RENDER_DPI, { layoutW: convertLayoutW, layoutH: convertLayoutH, layoutEm: convertLayoutEm });
    pageMap.set(page, b64);
    return b64;
  }, []);

  // プリフェッチ（前後1ページ）
  const prefetch = useCallback((path: string, page: number, max: number) => {
    [page - 1, page + 1].filter(p => p >= 0 && p < max).forEach(p => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        img.onload = () => {
          if (!cancelled) { setImgNaturalW(img.naturalWidth); setImgNaturalH(img.naturalHeight); }
        };
        ...
        getPageText(activePath, viewPage, scale)
          .then(r => { if (!cancelled && r.ok) setTextBlocks(r.blocks); })
          .catch(() => {});
        getPageLinks(activePath, viewPage, scale)
          .then(r => { if (!cancelled && r.ok) setPageLinks(r.links); })
          .catch(() => {});

        // 前後プリフェッチ
        prefetch(activePath, viewPage, total);
      } catch (e) {
        if (!cancelled) { console.error("[viewer] render failed:", e); setMainLoading(false); }
      }
    })();
  ...
  useEffect(() => {
    ...
    else { const a = new Array(activeInfo.page_count).fill(undefined); thumbCache.current.set(activePath, a); setThumbs([]); }
    let cancelled = false;
    (async () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const m = new Map(fileCoverThumbs); let changed = false;
      for (const f of fileList) {
        if (!m.has(f.path)) {
          try { m.set(f.path, await renderPage(f.path, 0, THUMB_DPI)); changed = true; } catch {}
        }
      }
  ...
  return (
    <div style={s.root}>
      <PageHeader>
        ...
        <div style={{ flex: 1 }} />
        <button style={{ ...s.zBtn, ...(showSearch ? s.btnOn : {}), marginRight: 4 }}
          onClick={() => setShowSearch(v => !v)} title="検索">🔍</button>
        <button style={{ ...s.zBtn, ...(infoOpen ? s.btnOn : {}), marginRight: 8 }}
          onClick={() => setInfoOpen(v => !v)} title="ファイル情報">ℹ</button>
        <div style={s.zoomRow}>
          <button style={s.zBtn} onClick={() => setZoom(z => Math.max(0.25, +(z - 0.25).toFixed(2)))}>−</button>
          <span style={s.zVal}>{Math.round(zoom * 100)}%</span>
          <button style={s.zBtn} onClick={() => setZoom(z => Math.min(4.0, +(z + 0.25).toFixed(2)))}>＋</button>
          ...
          <div style={s.filePane}>
            ...
            <div style={{ flex: 1, overflowY: "auto" }}>
              {fileList.map((f, i) => {
                ...
                return (
                  <button key={f.id} style={{ ...s.filePaneItem, ...(i === activeIdx ? s.filePaneItemOn : {}) }}
                    onClick={() => { setActiveIdx(i); setViewPage(0); setInfoOpen(false); }}>
                    <div style={s.filePaneThumbBox}>
                      {cover ? <img src={`data:image/jpeg;base64,${cover}`} style={s.filePaneThumbImg} alt="" /> : <span style={s.filePaneIcon}>📄</span>}
                    </div>
        ...
        <div style={s.thumbPane}>
          ...
          <div>
            {Array.from({ length: total }, (_, i) => {
              ...
              return (
                <button key={i} style={{ ...s.thumbItem, ...(i === viewPage ? s.thumbItemOn : {}) }}
          ...
          <div style={s.mainView}>
            <div style={s.viewScroll} ref={scrollRef}>
              <div style={s.viewInner}>
                {mainLoading && !mainImg && (
                  <div style={s.viewCenter}><div style={s.viewSpinner} /></div>
                )}
                {mainImg && (
                  <div style={{
                    position: "relative",
                    ...
                    {pageLinks.length > 0 && displayW > 0 && (
                      <div style={{ position: "absolute", inset: 0, width: displayW, height: displayH }}>
            ...
            {/* ページナビ */}
            <div style={s.pageNav}>
              <button style={s.navBtn} onClick={() => setViewPage(p => Math.max(0, p-1))} disabled={viewPage === 0}>◀</button>
              <span style={s.pageInfo}>{viewPage + 1} / {total}</span>
              <button style={s.navBtn} onClick={() => setViewPage(p => Math.min(total-1, p+1))} disabled={viewPage >= total-1}>▶</button>
            </div>
          </div>

          <InfoDrawer open={infoOpen} onClose={() => setInfoOpen(false)} info={activeInfo} filePath={activePath} fileName={fname} />
        </div>
      </div>
    </div>
  );
...
const ss: Record<string, React.CSSProperties> = {
  bar: { display: "flex", alignItems: "center", gap: 8, padding: "6px 12px", background: "var(--c-bgSub)", borderBottom: "1px solid var(--c-border)" },
  input: { flex: 1, maxWidth: 320, padding: "4px 8px", borderRadius: 4, border: "1px solid var(--c-border)", background: "var(--c-bg)", color: "var(--c-text)", fontSize: 13 },
  count: { fontSize: 11, color: "var(--c-textDim)", minWidth: 60, textAlign: "center" as const },
  navBtn: { width: 26, height: 26, cursor: "pointer", borderRadius: 4, border: "1px solid var(--c-border)", background: "var(--c-bgSub)", color: "var(--c-text)", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center" },
  close: { background: "transparent", border: "none", color: "var(--c-textDim)", cursor: "pointer", fontSize: 14 },
};

const ds: Record<string, React.CSSProperties> = {
  drawer: { position: "absolute", top: 0, right: 0, bottom: 0, width: 300, background: "var(--c-bgSub, #1a1f1b)", borderLeft: "1px solid var(--c-border)", display: "flex", flexDirection: "column", zIndex: 20, transition: "transform 0.22s cubic-bezier(0.4, 0, 0.2, 1)", boxShadow: "-4px 0 24px rgba(0,0,0,0.4)" },
  drawerHead: { display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", borderBottom: "1px solid var(--c-border)", position: "sticky", top: 0, background: "var(--c-bgSub, #1a1f1b)", zIndex: 1 },
  drawerTitle: { fontWeight: 700, fontSize: 13, color: "var(--c-text)" },
  drawerBody: { flex: 1, padding: "10px 12px", overflowY: "auto" },
  section: { fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", color: "var(--c-textDim)", marginBottom: 6, paddingBottom: 4, borderBottom: "1px solid var(--c-border)" },
  row: { display: "flex", alignItems: "flex-start", gap: 6, padding: "5px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" },
  ...
  copyBtn: { flexShrink: 0, background: "transparent", border: "1px solid var(--c-border)", borderRadius: 4, color: "var(--c-textDim)", fontSize: 11, cursor: "pointer", padding: "1px 5px", lineHeight: 1.4 },
  copyBtnDone: { background: "var(--c-accent, #3a7a4a)", color: "#fff", borderColor: "transparent" },
  closeBtn: { background: "transparent", border: "none", color: "var(--c-textDim)", fontSize: 14, cursor: "pointer", padding: "2px 6px" },
};
...
const s: Record<string, React.CSSProperties> = {
  ...
  thumbItem: { display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 15, background: "none", border: "2px solid transparent", cursor: "pointer" },
  thumbItemOn: { borderColor: "var(--c-accent)" },
  thumbN: { fontSize: 10, marginTop: 4, color: "#ccc" },
  thumbNOn: { color: "var(--c-accent)" },
  filePane: { width: 220, flexShrink: 0, display: "flex", flexDirection: "column", borderRight: "1px solid var(--c-border)", background: "var(--c-bgSide)" },
  filePaneItem: { display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", width: "100%", background: "transparent", border: "none", borderBottom: "1px solid rgba(255,255,255,0.05)", cursor: "pointer", overflow: "hidden" },
  filePaneItemOn: { background: "rgba(255,255,255,0.1)", borderLeft: "3px solid var(--c-accent)" },
  filePaneThumbBox: { width: 36, height: 48, flexShrink: 0, background: "#000", borderRadius: 2, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", boxShadow: "0 2px 4px rgba(0,0,0,0.3)" },
  ...
  filePaneName: { fontSize: 12, color: "var(--c-text)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontWeight: 500 },
  ...
  viewInner: { display: "flex", alignItems: "flex-start", justifyContent: "center", minWidth: "100%", padding: "24px 24px 0" },
  ...
  pageNav: { display: "flex", alignItems: "center", justifyContent: "center", gap: 16, padding: "8px", borderTop: "1px solid var(--c-border)", background: "var(--c-bgSub)" },
  navBtn: { width: 32, height: 32, cursor: "pointer", borderRadius: 4, border: "1px solid var(--c-border)", background: "var(--c-bgSub)", color: "var(--c-text)", fontSize: 14 },
