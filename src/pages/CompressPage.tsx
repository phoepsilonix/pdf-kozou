...
import {
  compressPdf, getTmpPath, renderPage, type CompressPreset, type CompressResponse, type PdfInfo,
} from "../lib/tauri";
...
const PRESETS: {
  ...
  { id:"maximum",    icon:"🔥",  label:"最大",    desc:"GC=3、sanitize,clean,merge-fonts,object-stream",    note:"⚠ 最大圧縮",   color:"#7a2020" },
];

// --- 追加：GS専用のプリセット定義 ---
const GS_PRESETS: {
  id: "Prepress" | "Printer" | "Ebook"; 
  icon: string; label: string; desc: string; note: string; color: string;
}[] = [
  { id:"Prepress", icon:"✨", label:"高品質(prepress)", desc:"300dpi / 低圧縮", note:"印刷・商業出版向け。画質を最優先", color:"#2a5a9a" },
  { id:"Printer",  icon:"🖨️", label:"プリント(printer)", desc:"300dpi / 中圧縮", note:"一般的な配布・印刷用。標準的", color:"#3a7a4a" },
  { id:"Ebook",    icon:"📱", label:"電子書籍,Web向け(ebook)", desc:"150dpi / 高圧縮", note:"画面閲覧・メール送信用。軽量化", color:"#7a5a1a" },
];
...
export function CompressPage({ filePath, pdfInfo, sourceFile, onDone, batchFiles }: Props) {
  ...
  const handlePreview = useCallback(async () => {
    ...
    try {
      ...
      const gsLog = await invoke<string>("run_gs_optimize", {
          gsPath: gsPath, input: inputFile, output: tmp, level: gsPreset
        });
  ...
  }, [currentSource, useGs, gsPath, inputFile, gsPreset, preset, mergeFonts, objectStream, pdfInfo, setError]);
  ...
  }, [useGs, gsPath, inputFile, filePath, gsPreset, preset, mergeFonts, objectStream, pickSave, setError, onDone]);
  ...
  }, [batchFiles, useGs, gsPath, gsPreset, preset, mergeFonts, objectStream, outDir, pickDir, setError]);
  ...
  if (phase === "result" && result) {
    ...
    return (
      <div style={c.root}>
        ...
        <div style={c.resultBody}>
          <div style={c.prevCol}>
            {preview ? <img src={`data:image/jpeg;base64,${preview}`} style={c.prevImg} alt="preview" /> : <div style={c.prevPh}>プレビューなし</div>}
          </div>
          <div style={c.statsCol}>
            <div style={{ ...c.statBig, color: gain ? "#4fe090" : "#ff6060" }}>{gain ? `−${pct}%` : `+${Math.abs(Number(pct))}%`}</div>
            <div style={c.statInfo}>
              <div style={c.statLabel}>ファイルサイズ変化</div>
              <div style={c.statVal}>{inMB} MB → <span style={{ color: gain ? "#4fe090" : "#ff6060", fontWeight: 700 }}>{outMB} MB</span></div>
            </div>
	    <div style={c.paramsBox}>
    {/* GSモードならGS専用バッジを表示 */}
    {useGs && (
      <span style={{...c.paramBadge, background: "var(--c-accentBg)", color: "var(--c-accent)", borderColor: "var(--c-accentBd)"}}>
	    ...
	    {p && !useGs && (
		    <>
	      {useGs && <span style={{...c.paramBadge, background:"var(--c-accentBg)", color:"var(--c-accent)"}}>Ghostscript: {gsPreset}</span>}
	...
	<div style={{ marginTop: 12, padding: 12, border: "1px dashed var(--c-accentBd)", borderRadius: 8, background: "var(--c-accentBg)11" }}>
	  <div style={{ fontSize: 11, color: "var(--c-accent)", fontWeight: 700, marginBottom: 6 }}>
	    🔗 連続圧縮モード
	  </div>
	    ...
	    ※ 現在の圧縮結果を「入力ファイル」として再セットします
            ...
            <div style={c.saveChoiceBox}>
              <div style={c.saveChoiceBtns}>
                <button style={c.btnSaveCompressed} onClick={handleSaveCompressed} disabled={saving}>
  ...
  return (
      ...
      <span style={c.fileSub} title={fname}>
        ...
        {gsAvailable && (
          <div style={{ display: "flex", gap: 4, marginLeft: 16 }}>
            <button style={{ padding: "2px 10px", fontSize: 11, borderRadius: 4, cursor: "pointer", border: "1px solid var(--c-borderHi)", background: !useGs ? "var(--c-accent)" : "transparent", color: !useGs ? "#000" : "var(--c-textSub)", fontFamily: F }} onClick={() => setUseGs(false)}>
              標準 (MuPDF)
            </button>
            <button style={{ padding: "2px 10px", fontSize: 11, borderRadius: 4, cursor: "pointer", border: "1px solid var(--c-borderHi)", background: useGs ? "var(--c-accent)" : "transparent", color: useGs ? "#000" : "var(--c-textSub)", fontFamily: F }} onClick={() => setUseGs(true)}>
              プロ (GS)
            </button>
          </div>
        )}
	...
	{/* 連携中（初期ファイルと異なる）場合のみリセットボタンを表示 */}
        ...
        {!useGs ? (
          <>
            <div style={c.presetGrid}>
              {PRESETS.map((p) => (
                <button key={p.id} onClick={() => setPreset(p.id)} style={{ ...c.card, ...(preset === p.id ? { borderColor: p.color, background: p.color + "22" } : {}) }}>
            ...
            <div style={c.optRow}>
              <label style={c.optLabel}><input type="checkbox" checked={objectStream} onChange={(e) => setObjectStream(e.target.checked)} style={{ marginRight: 6 }} />オブジェクトのストリーム圧縮</label>
              <span style={c.optHint}>{objectStream ? "有効 (MuPDFのデフォルト設定を上書き)" : "無効"}</span>
            </div>
            <div style={c.optRow}>
              <label style={c.optLabel}><input type="checkbox" checked={mergeFonts} onChange={(e) => setMergeFonts(e.target.checked)} style={{ marginRight: 6 }} />CIDフォント統合 (merge-fonts)</label>
              <span style={c.optHint}>{mergeFonts ? "有効 (フォントを再利用して削減)" : "無効"}</span>
            </div>
          </>
        ) : (
          <>
      ...
      <div style={c.presetGrid}>
        {GS_PRESETS.map((p) => (
          <button 
            ...
            style={{ ...c.card, ...(gsPreset === p.id ? { borderColor: p.color, background: p.color + "22" } : {}) }}
          >
            <span style={c.cardIcon}>{p.icon}</span>
            <span style={c.cardLabel}>{p.label}</span>
            <span style={c.cardDesc}>{p.desc}</span>
      ...
      <div style={{ padding: "24px 22px", fontSize: 12, color: "var(--c-textDim)", lineHeight: 1.6 }}>
        <div style={{ marginBottom: 4, color: "var(--c-textSub)", fontWeight: 700 }}>Ghostscript プロモードについて</div>
	  ...
	  {isBatch ? (
		...
		<button style={c.dirPickBtn} onClick={pickDir}>フォルダ選択</button>
	      </div>
	      <button 
		...
		disabled={!outDir || (useGs && !gsPath)}
	      >
		{useGs ? `プロモードで一括圧縮 (${batchFiles!.length}個)` : `一括圧縮を開始 (${batchFiles!.length}個)`}
	      </button>
	    </div>
	  ) : (
...
const c: Record<string, React.CSSProperties> = {
  ...
  header:     { display: "flex", alignItems: "center", gap: 12, padding: "12px 22px", borderBottom: `1px solid var(--c-border)`, flexShrink: 0 },
  ...
  fileSub:    { fontSize: 12, color: "var(--c-textSub)", maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" },
  ...
  optLabel:   { display: "flex", alignItems: "center", fontSize: 13, color: "var(--c-text)", cursor: "pointer" },
  optHint:    { fontSize: 11, color: "var(--c-textSub)" },

  execArea:      { padding: "24px 22px", borderTop: `1px solid var(--c-border)`, display: "flex", justifyContent: "center", background: "var(--c-bg)", flexShrink: 0 },
  singleExecBox: { width: "100%", maxWidth: 400 },
  btnExec:       { width: "100%", padding: "14px", background: "var(--c-accentBg)", border: `2px solid var(--c-accentBd)`, borderRadius: 10, color: "var(--c-accent)", fontWeight: 700, fontSize: 15, cursor: "pointer", fontFamily: F },
  ...
  dirPath:       { flex: 1, padding: "9px 12px", background: "var(--c-bgCard)", border: `1px solid var(--c-border)`, borderRadius: 8, fontSize: 12, color: "var(--c-textSub)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" },
  dirPickBtn:    { padding: "0 16px", background: "var(--c-bgHover)", border: `1px solid var(--c-borderHi)`, borderRadius: 8, color: "var(--c-text)", fontSize: 12, cursor: "pointer", fontFamily: F },
  ...
  prevImg:    { maxWidth: 260, maxHeight: 360, borderRadius: 6, border: `1px solid var(--c-border)`, background: "#fff", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" },
  ...
  btnSaveCompressed: { flex: 1, padding: "14px 10px", background: "var(--c-accentBg)", border: `2px solid var(--c-accentBd)`, borderRadius: 9, cursor: "pointer", fontFamily: F, transition: "all 0.12s" },
  btnSaveOriginal:   { flex: 1, padding: "14px 10px", background: "var(--c-bgHover)", border: `1px solid var(--c-borderHi)`, borderRadius: 9, cursor: "pointer", fontFamily: F, transition: "all 0.12s" },
  ...
  btnChain: {
    flex: 1, padding: "8px 12px", background: "var(--c-accent)", color: "#000", 
    border: "none", borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: F
  },
