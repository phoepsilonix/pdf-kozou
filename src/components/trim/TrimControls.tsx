        ...
        <div style={s.cross}>
          ...
          <div style={s.crossMid}>
            ...
            <div style={s.pageBox}>
              ...
              <span style={{ ...s.pageSize, color: C.accent }}>{trimW} × {trimH}</span>
...
const s: Record<string, React.CSSProperties> = {
  panel: {
    ...
    padding:"18px 14px", background:C.bgCard, color:C.text,
    fontFamily:F, fontSize:14, height:"100%", overflowY:"auto",
  },
  section:  { display:"flex", flexDirection:"column", gap:10 },
  heading:  { margin:0, fontSize:11, letterSpacing:"0.1em", textTransform:"uppercase", color:C.textDim },
  ...
  pageSize: { fontSize:12, color:C.textSub },
  pageUnit: { fontSize:10, color:C.textDim },
  arrow:    { fontSize:14, color:C.textDim },

  field:      { display:"flex", flexDirection:"column", alignItems:"center", gap:4 },
  fieldLabel: { fontSize:12, color:C.textSub, fontWeight:600 },
  input: {
    width:76, padding:"7px 6px",
    background:C.bg, border:`1px solid ${C.borderHi}`, borderRadius:7,
    color:C.text, fontSize:26, textAlign:"center",
    outline:"none", fontFamily:F, fontWeight:700,
  },
  unit: { fontSize:10, color:C.textDim },

  chips: { display:"flex", gap:6 },
  chip: {
    padding:"6px 12px", borderRadius:16,
    border:`1px solid ${C.borderHi}`, background:C.bgCard,
    color:C.textSub, cursor:"pointer", fontSize:12,
    fontFamily:F, transition:"all 0.12s",
  },
  chipOn: { background:C.accentBg, borderColor:C.accentBd, color:C.accent },

  actions:    { display:"flex", gap:8 },
  btnReset:   { padding:"10px 14px", background:"transparent", border:`1px solid ${C.borderHi}`, borderRadius:7, color:C.textSub, cursor:"pointer", fontFamily:F, fontSize:13 },
  btnApply:   { flex:1, padding:"10px 0", background:C.accentBg, border:`1px solid ${C.accentBd}`, borderRadius:7, color:C.accent, fontWeight:700, cursor:"pointer", fontFamily:F, fontSize:14 },
  btnDisabled:{ opacity:0.4, cursor:"not-allowed" },
  hint:       { margin:0, fontSize:11, color:C.textDim, lineHeight:1.6 },
};
