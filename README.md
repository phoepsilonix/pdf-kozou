...
またGhostScriptがインストールされているフォルダなどを手動で指定する機能も追加しました。About画面で設定できます。  
...
About画面から設定できます。「フォルダから検索」を押して、GhostScriptのあるフォルダを選び、「確認して保存」を押してみてください。

試験的機能
[隠しテキスト検出](hidden-text-spec.md)
...
トリミング、CropBoxをもちいられたPDFのファイルサイズ削減、圧縮は、そこまで有能ではありません。(〜v1.8.1まで)  
そういうタイプのファイルはGhostScript連携の圧縮を試してみてください。  
v1.8.2から、圧縮オプションにCropbox領域外にapply_redactionsを利用する設定を加えました。  
(GhostScriptのほうが綺麗に圧縮できるとは思います。安全のため標準でトリミング領域の上下左右100ptを残しています。境界にある部品が消える可能性があるためです。この数値はユーザーが指定できます。)  
v1.8.4より、画像のダウンサンプリングを追加しました。必要解像度より15%以上大きいJpeg画像や無圧縮タイプの画像を再圧縮します。  
おすすめの圧縮方法はGhostScriptで圧縮後、MuPDFとの連携圧縮でオブジェクトストリームの圧縮のみオンにする流れです。
...
[コマンドライン詳細](cli-reference.md)

v1.8.0からサイドカー構成廃止。
pdf-kozou-coreコマンドラインはv1.7.20以前のパッケージから取得または自力でビルドをしてください。
...
7zip版のアーカイブのバイナリ(v1.4.8〜)が使用できる場合には、そちらもご検討ください。(`webkit2gtk-4.1`があれば、7-zip版アーカイブのバイナリも動作するかもしれません。Ubuntuランナーでビルドしているバイナリですが、Arch系でも動いています。)
...
v1.8.0以降は、pdf-kozou-coreを組み込む形に変えていますので、パッケージとしてはpdf-kozou-coreを含みません。  
CLIとして用いたい方は`cargo build --release --bin pdf-kozou-core`でビルドしてください。
