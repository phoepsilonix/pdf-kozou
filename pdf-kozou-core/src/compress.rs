...
/// (reason=="buried")のみに限定し、閾値未満の部分的な重なりは、ここでは
/// 無傷のまま保持する — 誤って可視テキストまで消してしまう方が実害が
/// 大きいため、安全側に倒す。1up の画像化(kozou_compose_image_pdf_keep_text)
/// では、部分的に隠れているテキストは C 側が、隠れていた部分だけを覆う
/// 小さなパッチ画像(SMask付き)を最前面に重ねて隠し直す
/// (kozou_keeptext_add_partial_covers)。面付け(N-up/見開き)のキープ
/// テキスト版は、この隠し直しにはまだ対応していない。
///
