...
pub fn render(req: &RenderRequest) -> Result<RenderResponse> {
    ...
    let page = doc.load_page(req.page_index).map_err(|e| CoreError::MuPdf(e.to_string()))?;
    ...
    let pm = page.to_pixmap(&matrix, &mupdf::Colorspace::device_rgb(), false, false)
    ...
    if let Some(out_path) = &req.output {
        ...
        return Ok(RenderResponse {
            ok: true, image_b64: String::new(), format: format.to_string(),
            width_px: pm.width() as u32, height_px: pm.height() as u32,
            page_w_pt: bounds.x1 - bounds.x0, page_h_pt: bounds.y1 - bounds.y0,
            dpi: req.dpi, output: Some(out_path.clone()),
        });
    }
    ...
    Ok(RenderResponse {
        ok: true, image_b64, format: format.to_string(),
        width_px: pm.width() as u32, height_px: pm.height() as u32,
        page_w_pt: bounds.x1 - bounds.x0, page_h_pt: bounds.y1 - bounds.y0,
        dpi: req.dpi, output: None,
    })
}

/// SVG レンダリング — フォントアウトラインをベクターパスとして保持
fn render_svg(req: &RenderRequest, page: &mupdf::Page, bounds: &mupdf::Rect) -> Result<RenderResponse> {
    ...
    page.run(&dev, &mupdf::Matrix::IDENTITY).map_err(|e| CoreError::MuPdf(e.to_string()))?;
    writer.end_page(dev).map_err(|e| CoreError::MuPdf(e.to_string()))?;
    drop(writer);

    if req.output.is_some() {
        return Ok(RenderResponse {
            ok: true, image_b64: String::new(), format: "svg".into(),
            width_px: (page_w * req.dpi as f32 / 72.0) as u32,
            height_px: (page_h * req.dpi as f32 / 72.0) as u32,
            page_w_pt: page_w, page_h_pt: page_h, dpi: req.dpi,
            output: req.output.clone(),
        });
    }

    let svg_bytes = std::fs::read(&tmp).map_err(|e| CoreError::Io(e))?;
    ...
    Ok(RenderResponse {
        ...
        width_px: (page_w * req.dpi as f32 / 72.0) as u32,
        height_px: (page_h * req.dpi as f32 / 72.0) as u32,
        page_w_pt: page_w, page_h_pt: page_h, dpi: req.dpi, output: None,
    })
}
